        ...
        <el-scrollbar style="width: 100%;text-align: center;">
            <el-form class="tunnel-edit-form" ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="auto"
                ...
                <el-form-item label="节点选择" prop="node">
                    ...
                    <el-select v-model="ruleForm.node" placeholder="请选择节点">
                ...
                <el-form-item label="端口类型" prop="type" required>
                    <el-select v-model="ruleForm.type" placeholder="请选择端口类型">
                        <el-option v-for="item in realNetworkTypes" :key="item" :label="item" :value="item">
...
import { NodeInfo, TunnelCreate } from '@/api/types'
import { chmlfrpNodes, chmlfrpTunnelCreate, chmlfrpTunnelUpdate, chmlfrpUserInfo } from '@/api';
...
onMounted(() => {
    ...
    chmlfrpNodes().then((data) => {
        ...
        ruleForm.node = curNode.value['name']
        randomName()
        randomPort()
...
let rules = reactive<FormRules<TunnelCreate>>({
    token: [
        {
            required: true,
            message: '用户令牌必须存在',
...
const realNetworkTypes = computed(() => {
    if (nodes.value.length <= 0) {
        return networkTypes
    }


    const item = nodes.value.find(item => {
        if (item['name'] === ruleForm['node']) {
            return true
        }
    })
    if (!item) {
...
function update() {
    let createObject = Object.assign(ruleForm)
    ...
    ruleFormRef.value?.validate((valid, fields) => {
        ...
        if (valid) {
            chmlfrpTunnelCreate(createObject).then(data => {
                if (data.code === 200) {
